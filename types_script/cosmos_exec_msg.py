...
def main(tx_id, tx_type, message, ids):
    ...
    try:
        # Define the values
        msg_num = len(message['msgs'])
        message = json.dumps(message)

        # Edit the query that will be loaded to the database
        query = """
        INSERT INTO cosmos_exec_msg (tx_id, tx_type, receive_address_id, msg_num, message_info) VALUES (%s, %s, %s, %s, %s);
        """

        values = (tx_id, tx_type, ids['grantee_id'], msg_num, message)
