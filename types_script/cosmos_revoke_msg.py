...
'''**********************************************************************************
                                                                                    *
Project Name: cosmos_revoke_msg.py                                *
                                                                                    *
...
def main(tx_id, message_no, transaction_no, tx_type, message, ids):
    ...
    try:
        # Define the values
        msg_type_url = message['msg_type_url']
        message = json.dumps(message)
        comment = f'This is number {message_no} message in number {transaction_no} transaction '

        # Edit the query that will be loaded to the database
        query = """
        INSERT INTO cosmos_revoke_msg (tx_id, tx_type, send_address_id, receive_address_id, msg_type_url, message_info, comment) VALUES (%s, %s, %s, %s, %s, %s, %s);
        """

        values = (tx_id, tx_type, ids['granter_id'], ids['grantee_id'], msg_type_url, message, comment)
